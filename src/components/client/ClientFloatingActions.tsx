...
import { MessageCircle, Mail, Copy, Edit3, X, Zap } from 'lucide-react'
...
export function ClientFloatingActions({ phone, email, cpf, onEdit, onCopyCpf }: ClientFloatingActionsProps) {
  ...
  return (
    <div ref={menuRef} className="fixed bottom-6 right-6 z-40 flex flex-column align-items-end gap-3 pointer-events-none">
      <div
        className={cn(
        ...
        {actions.map((action, index) => {
          ...
          return (
            <div key={action.id} className="flex align-items-center gap-2 group">
              <span
                className={cn(
                  'px-2.5 py-1 rounded bg-[var(--color-text-primary)] text-white text-xs font-sans font-semibold shadow-md whitespace-nowrap transition-all duration-200 opacity-0 scale-95 origin-right translate-x-2 pointer-events-none',
              ...
              <button
                ...
                className={cn(
                  'w-12 h-12 rounded-full bg-white border border-[var(--color-border)] neo-btn flex align-items-center justify-content-center transition-all duration-200 pointer-events-auto',
                  action.color
                )}
      ...
      <button
        ...
        className={cn(
          'w-14 h-14 rounded-full flex align-items-center justify-content-center neo-btn text-white transition-all duration-300 transform active:scale-95 pointer-events-auto',
          isOpen
            ? 'bg-[var(--color-text-primary)] hover:bg-slate-700 rotate-90 scale-95'
