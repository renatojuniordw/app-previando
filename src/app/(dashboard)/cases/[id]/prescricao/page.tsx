  ...
  return [
    {
      label: 'Decadência',
      fundamentacao:
        'Art. 103 da Lei 8.213/91 — prazo de 10 anos para revisão do ato de concessão do benefício',
    ...
    },
    {
      ...
      fundamentacao:
        'Art. 103-A da Lei 8.213/91 — prazo de 5 anos para cobrança de diferenças de benefício',
...
export default function PrescricaoPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 p-8">
      <div>
        <h2 className="font-serif text-2xl font-bold text-slate-900">Prescrição e Decadência</h2>
      ...
      {/* Formulário */}
      <Card variant="light" className="space-y-5 p-6">
        <CardHeader
          title={
        ...
        <div className="animate-fade-in space-y-4">
          ...
          {prazos.map((prazo) => {
            ...
            return (
              <Card
                key={prazo.label}
                variant="light"
                className={`border p-6 ${cfg.border} ${prazo.status === 'critical' || prazo.status === 'expired' ? 'ring-1 ring-inset ' + cfg.border : ''}`}
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 items-end gap-3 sm:flex-col sm:gap-1">
                    ...
                    <div className={`rounded-lg px-3 py-2 text-right ${cfg.bg}`}>
                      <p className="text-xs text-slate-500">
                        {prazo.daysLeft >= 0 ? 'Restam' : 'Expirou há'}
                      </p>
                      <p className={`font-sans text-xl font-bold ${cfg.color}`}>
          ...
          <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <p className="text-xs leading-relaxed text-amber-800">
              <strong>Atenção:</strong> Os prazos podem ser suspensos ou interrompidos por causas
              legais (ex.: incapacidade civil, requerimento administrativo, ação judicial).
              Verifique sempre o caso concreto antes de concluir pelo decurso do prazo.
            </p>
          </div>
        </div>
      )}
      ...
      {!dataEvento && (
        <Card variant="light" className="p-10 text-center">
          <Clock className="mx-auto mb-3 h-10 w-10 text-slate-300" aria-hidden="true" />
