    ...
    <div className="space-y-8 mt-6">
      ...
      <div className="border-t border-slate-150 pt-6">
        <div className="flex flex-wrap align-items-center justify-content-between gap-4 mb-4">
          <h4 className="font-serif font-bold text-lg text-slate-900 flex align-items-center gap-2">
            <User className="w-5 h-5 text-[var(--color-primary)]" aria-hidden="true" />
            Dados do Segurado
          </h4>
          <button
            onClick={onExportCSV}
            className="neo-btn neo-btn-primary text-xs transition-all flex align-items-center gap-1.5 focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-4">
          ...
          <div className="bg-[var(--color-card-inner)] border border-[var(--color-border)] rounded-xl p-4">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap align-items-center justify-content-between gap-4 border-t border-slate-100 pt-6">
          <div className="flex flex-wrap align-items-center gap-2">
            <h4 className="font-serif font-bold text-lg text-slate-900 flex align-items-center gap-2">
              <Briefcase className="w-5 h-5 text-[var(--color-primary)]" aria-hidden="true" />
              Histórico de Vínculos Empregatícios
            </h4>
            <span className="font-sans text-xs text-slate-500 font-medium bg-[var(--color-card-inner)] px-2.5 py-1 rounded-full whitespace-nowrap">
              {data.periodos?.length ?? 0} {data.periodos?.length === 1 ? 'vínculo' : 'vínculos'}
            </span>
          </div>
          <div className="flex flex-wrap align-items-center gap-2">
            <button onClick={expandAll} className="text-xs font-semibold text-slate-600 hover:text-slate-800 bg-[var(--color-card-inner)] px-2.5 py-1.5 rounded transition-all focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none">
              Expandir Todos
            </button>
            <button onClick={collapseAll} className="text-xs font-semibold text-slate-600 hover:text-slate-800 bg-[var(--color-card-inner)] px-2.5 py-1.5 rounded transition-all focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none">
              Recolher Todos
            </button>
            <button onClick={onAddPeriod} className="bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white font-sans font-semibold text-xs px-3 py-1.5 rounded-lg shadow-sm flex align-items-center gap-1.5 transition-colors focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:outline-none">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 neo-input-neo rounded-lg text-sm font-sans focus:outline-none  focus-visible:ring-2 transition-all placeholder:text-slate-400"
          />
        </div>

        {filteredPeriodos.length === 0 ? (
          <div className="text-center py-12 bg-[var(--color-card-inner)] border border-[var(--color-border)] rounded-xl">
...
function EditableField({ label, value, prompt, currentValue, onSave }: {
  ...
  return (
    <div className="bg-[rgba(242,232,228,0.2)] border border-[#F5D0C3]/30 rounded-xl p-4 relative group">
      ...
      <div className="flex align-items-center justify-content-between gap-2">
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 hover:text-[var(--color-primary)] transition-opacity p-0.5"
