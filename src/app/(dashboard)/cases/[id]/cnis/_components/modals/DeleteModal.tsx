  ...
  return (
    <Modal open={open} onClose={onClose} title="EXCLUIR EXTRATO DO CNIS?">
      <div className="space-y-4 font-sans text-sm text-slate-600 leading-relaxed">
        <div className="border border-red-200 bg-red-50 rounded-xl p-4 flex align-items-start gap-3 text-red-800">
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onConfirm} disabled={deleting} className="flex-1 neo-btn neo-btn-danger flex-1 text-center transition-colors font-sans text-sm flex align-items-center justify-content-center gap-2 focus-visible:ring-2 focus-visible:ring-red-500">
            {deleting ? <><Loader2 className="w-4 h-4 animate-spin" />Excluindo…</> : 'SIM, EXCLUIR TUDO'}
          </button>
          <button onClick={onClose} disabled={deleting} className="flex-1 neo-btn-outline flex-1 text-center transition-colors font-sans text-sm">
            CANCELAR
          </button>
        </div>
      </div>
