...
export function CnisUploadDropzone({ isDragging, uploading, isProcessing, onDragOver, onDragLeave, onDrop, onUploadClick }: Props) {
  return (
    <div
      ...
      onDrop={onDrop}
      className={cn(
        "py-20 flex flex-col items-center justify-center border-2 border-dashed rounded-2xl text-center transition-all duration-300",
        isDragging
          ? 'border-amber-500 bg-amber-50/20 scale-[0.99] shadow-inner'
          : 'border-slate-300 bg-white hover:bg-slate-50/50 hover:border-slate-350 shadow-sm'
      )}
    >
      <div className="w-16 h-16 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-center mb-5 shadow-xs text-slate-550">
        <FileText className="w-8 h-8" />
      </div>
      <h3 className="font-serif font-bold text-xl text-slate-900 mb-2">Nenhum CNIS Enviado</h3>
      <p className="font-sans text-sm text-slate-550 mb-7 max-w-md mx-auto px-6 leading-relaxed font-medium">
