      ...
      <div className="flex items-center gap-4 border-b border-slate-200 pb-6">
        ...
        <div>
          ...
          <p className="font-sans text-sm text-slate-500 mt-0.5 font-medium">
            Adicione múltiplos segurados à sua base de dados a partir de uma planilha Excel ou CSV.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border-t border-slate-100 pt-4 flex flex-col items-center space-y-2.5">
              <p className="font-sans text-[10.5px] text-slate-400 font-bold uppercase tracking-wider text-center">
                Colunas aceitas: Nome, CPF, Data Nascimento, Telefone, Email, Prioridade, Observações
              </p>
              <a
        ...
        <div className="space-y-6">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="pb-3 font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider">Nome</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {preview.erros.map((err, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                        ...
                        <td className="py-2.5 pr-4 font-sans font-semibold text-xs text-slate-800">{err.nome || '—'}</td>
                        <td className="py-2.5">
          ...
          <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 shadow-sm gap-4">
            <p className="font-sans text-xs text-slate-500 font-medium">
              {preview.validos > 0
                ? `${preview.validos} cliente${preview.validos !== 1 ? 's' : ''} pronto${preview.validos !== 1 ? 's' : ''} para ser adicionado.`
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                Importar {preview.validos} Clientes
              </Button>
            </div>
          </div>
        </div>
        ...
        <Card variant="light" className="text-center py-16 border-slate-200/80">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="space-y-1">
              ...
              <p className="font-sans text-xs text-slate-400 font-medium">Cadastrando segurados no banco de dados...</p>
            </div>
            {/* Progress bar */}
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-sans text-xs text-slate-500 mt-0.5 font-medium">
                  {importResult.created} de {importResult.total} registros foram salvos.
                </p>
              </div>
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white border border-green-200 bg-green-50/[0.08] rounded-xl p-5 shadow-sm text-center">
              ...
              <p className="font-mono font-bold text-2xl text-green-750">{importResult.created}</p>
              <p className="font-sans text-[10px] text-green-600 font-bold uppercase tracking-wider mt-1">Importados</p>
            </div>
            <div className="bg-white border border-yellow-250 bg-amber-50/15 rounded-xl p-5 shadow-sm text-center">
              ...
              <p className="font-sans text-[10px] text-amber-600 font-bold uppercase tracking-wider mt-1">Duplicatas</p>
            </div>
            <div className="bg-white border border-red-200 bg-red-50/10 rounded-xl p-5 shadow-sm text-center">
              ...
              <p className="font-mono font-bold text-2xl text-red-750">
          ...
          {importResult.results.filter((r) => r.status !== 'created').length > 0 && (
            <Card variant="light" className="p-5 border-slate-200/80">
              <h3 className="font-serif font-bold text-sm text-slate-800 border-b border-slate-100 pb-3 mb-4">Detalhamento dos Eventos</h3>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="pb-3 font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider">Linha</th>
                      <th className="pb-3 font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider">Nome</th>
                      <th className="pb-3 font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider">Status</th>
                      <th className="pb-3 font-sans font-bold text-[10px] text-slate-400 uppercase tracking-wider">Detalhe</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {importResult.results.map((r, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                        <td className="py-2.5 pr-4 font-mono text-xs text-slate-500 font-bold">{r.row}</td>
                        <td className="py-2.5 pr-4 font-sans font-semibold text-xs text-slate-800 truncate max-w-[150px]">{r.nome ?? '—'}</td>
                        ...
                        <td className="py-2.5 font-sans text-xs text-slate-500 font-medium">{r.message ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 shadow-sm gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => router.push('/clients/list')} className="bg-slate-900 hover:bg-slate-850 text-white border-slate-900 font-sans font-bold text-xs h-9.5 shadow-sm">
                Ir p/ Lista
              </Button>
            </div>
          </div>
