...
import { createFeeWorker, scheduleFeeJob } from './fee-worker'
...
scheduleFeeJob(redis).catch((err) => logger.error('Failed to schedule fee job', err))

logger.info('BullMQ workers started — CNIS processing + audit log + email + deadline + fee notifications')
